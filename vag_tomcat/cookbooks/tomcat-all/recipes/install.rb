...
template node['tomcat-all']['install_directory'] + "/tomcat/bin/catalina.sh" do
  ...
  notifies :create, "template[#{node['tomcat-all']['install_directory']}/tomcat/conf/context.xml]", :immediately
end

# Tomcat context with Datasources
template node['tomcat-all']['install_directory'] + "/tomcat/conf/context.xml" do
  source 'context.xml.erb'
  notifies :create, "template[#{node['tomcat-all']['install_directory']}/tomcat/conf/tomcat-users.xml]", :immediately
end

# Tomcat users to enable manager
template node['tomcat-all']['install_directory'] + "/tomcat/conf/tomcat-users.xml" do
  source 'tomcat-users.xml.erb'
  notifies :create, "template[#{node['tomcat-all']['install_directory']}/tomcat/conf/catalina.properties]", :immediately
end

# Add /vagrant/instance-config/ to classpath
template node['tomcat-all']['install_directory'] + "/tomcat/conf/catalina.properties" do
